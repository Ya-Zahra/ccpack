...
begin
  ...
  if (ClassType <> TBox) and not (csDesignInstance in ComponentState) then begin
...
begin
  if Child is TControl then
    ...
    for i := 0 to ComponentCount - 1 do
      if not Components[i].HasParent then begin
        Inc(J);
        if J = Order then begin
...
begin
  inherited Create(AOwner);
  if (ClassType <> TControlGroupBox) and not (csDesignInstance in ComponentState) then begin
    if not InitInheritedComponent(Self, ClassType.ClassParent) then
      raise EResNotFound.CreateFmt(sResNotFound, [ClassName]);
    if Assigned(FOnCreate) then
      try
...
begin
  inherited Create(AOwner);
  if (ClassType <> TControlScrollBox) and not (csDesignInstance in ComponentState) then begin
    if not InitInheritedComponent(Self, ClassType.ClassParent) then
      raise EResNotFound.CreateFmt(sResNotFound, [ClassName]);
    if Assigned(FOnCreate) then
      try
