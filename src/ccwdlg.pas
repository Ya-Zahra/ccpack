...
{                                                       }
{     Bugfixes and Delpih 2006 adaptation by:           }
{       Thomas Mueller                                  }
...
begin
  ...
  while (Result <> nil) do begin
    if AnsiCompareText(Result.Text, AClassName) = 0 then
      Break;
...
var
  ...
  Index: Integer;
begin
  if not Assigned(CCtree.Selected) or not Assigned(CCtree.Selected.Data) then
    Exit;
...
begin
  Nodes := CCtree.Items;
  for i := 0 to GetCustomContainerClassListCount - 1 do begin
    AClass := GetCustomContainerClass(i);
    if IsBaseContainer(AClass) <> -1 then
      Nodes.AddObject(nil, AClass.ClassName, AClass)
    else
      Nodes.AddChildObject(GetContainerNode(AClass.ClassParent.ClassName), AClass.ClassName, AClass);
  end;
end;
...
begin
  inherited;
  LoadNodes;
  Node := CCtree.Items.GetFirstNode;
